from tkinter import Tk, Frame, BOTH, X, IntVar, messagebox
...
class Example(Frame):
    ...
    def initUI(self):
        ...
        for i in range(11):
            ...
            if i == 9:
                ...
                Radiobutton(self, value=9, text="Không biết", variable=v[i]).place(x=xx, y=j + 100)
                j = j + 160
                continue
            ...
            Radiobutton(self, value=9, text="Không biết", variable=v[i]).place(x=xx, y=j + 40)
            j = j + 100
    ...
    def onClick(self, value, button):
        ...
        if p == None:
            messagebox.showinfo("Error","hello")
        # print(p)

        label = Label(self, text=p.call_again(), foreground="orange", font="Helvetica 18 italic").place(x=300, y=600)


root = Tk()
root.geometry("550x700+300+0")
app = Example(root)
root.mainloop()